...
def create_encoder(code_dim=100, batchsize=64, input_chn=3, input_size=64,
    ...
    layers.extend(ld.Linear('ip', layers[-1].top[0], decode_dim, bias_term=False,
                            filler_spec=filler_spec))
    layers.extend(ld.Reshape('reshape_ip', layers[-1].top[0],
                             [batchsize, chn[0], first_feat_size, first_feat_size]))
    layers.extend(ld.BatchNorm('ip_act', layers[-1].top[0],
                               moving_average_fraction=0.9))
    if durelu:
        layers.extend(ld.DuReLU('ip_act', layers[-1].top[0]))
    else:
        layers.extend(ld.ReLU('ip_act', layers[-1].top[0], leaky))
    # deconv layers
    for i in xrange(1, depth):
    ...
    # output 
    layers.extend(ld.Deconv('deconv_data', layers[-1].top[0], input_chn, kernel_size,
                            stride, pad, bias_term=True, filler_spec=filler_spec))
    if tanh is True:
        layers.extend(ld.Tanh('tanh_data', layers[-1].top[0]))
