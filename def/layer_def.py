...
def Upsample(name, bottom, factor):
    top_name = name
    mid_name = name + '_height'
    # dimension of height
    layer_h = caffe_pb2.LayerParameter()
    layer_h.name = mid_name
    layer_h.type = 'Tile'
    layer_h.bottom.extend([bottom])
    layer_h.top.extend([mid_name])
    layer_h.tile_param.axis = 2
    layer_h.tile_param.tiles = factor
    # dimension of width
    layer_w = caffe_pb2.LayerParameter()
    layer_w.name = name
    layer_w.type = 'Tile'
    layer_w.bottom.extend([mid_name])
    layer_w.top.extend([top_name])
    layer_w.tile_param.axis = 3
    layer_w.tile_param.tiles = factor
    return [layer_h, layer_w]
...
def ELU(name, bottom, alpha=1.0):
    layer = caffe_pb2.LayerParameter()
    layer.name = name + '_elu'
    layer.type = 'ELU'
    layer.bottom.extend([bottom])
    layer.top.extend([bottom])
    if alpha != 1.0:
        layer.elu_param.alpha = alpha
    return [layer]
...
def Reduction(name, bottom, axis=0):
    layer = caffe_pb2.LayerParameter()
    layer.name = name
...
def L1Loss(name, bottoms, samplewise=False):
    top_name = name
    diff_name = name + '_diff'
    abs_name = diff_name + '_abs'
    # diff: bottom[0] - bottom[1]
    layer_diff = caffe_pb2.LayerParameter()
    layer_diff.name = diff_name
    layer_diff.type = 'Eltwise'
    layer_diff.bottom.extend(bottoms)
    layer_diff.top.extend([diff_name])
    layer_diff.eltwise_param.operation = caffe_pb2.EltwiseParameter.SUM
    layer_diff.eltwise_param.coeff.extend([1,-1])
    # abs: |bottom[0] - bottom[1]|
    layer_abs = caffe_pb2.LayerParameter()
    layer_abs.name = abs_name
    layer_abs.type = 'AbsVal'
    layer_abs.bottom.extend([diff_name])
    layer_abs.top.extend([abs_name])
    # sum: 1 / num * \sum |bottom[0] - bottom[1]|
    layer_sum = caffe_pb2.LayerParameter()
    layer_sum.name = top_name
    layer_sum.type = 'SumLoss'
    layer_sum.bottom.extend([abs_name])
    layer_sum.top.extend([top_name])
    # sum samplewise
    if samplewise:
        layer_redu = Reduction(name+'_samplewise', abs_name, axis=1)
        return [layer_diff, layer_abs, layer_sum] + layer_redu
    return [layer_diff, layer_abs, layer_sum]
