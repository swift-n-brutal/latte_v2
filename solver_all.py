...
class Solver(object):
    ...
    HISTORY_SOLVER_TYPE = ['SGD', 'Adam']
    HISTORY2_SOLVER_TYPE = ['RMSProp', 'Adam'] 
    ...
    def _update_sgd_gpu(self, param, history):
    ...
    def _update_adam_gpu(self, param, history, history2):
        self.accum_mult = self.accum_mult * self.mom + 1.0
        self.accum_mult2 = self.accum_mult2 * self.mom2 + 1.0
        for (p, h, h2) in zip(param, history, history2):
            if p is not None:
                for (bp, bh, bh2) in zip(p, h, h2):
                    # param.diff = param.diff + param.data * decay
                    if self.decay != 0:
                        axpby(DTYPE(1.), bp.gpu_diff,
                              DTYPE(self.decay), bp.gpu_data, bp.gpu_diff)
                    # history.data = history.data * mom + param.diff
                    axpby(DTYPE(self.mom), bh.gpu_data,
                          DTYPE(1.), bp.gpu_diff, bh.gpu_data)
                    # param.diff = param.diff ^ 2
                    sqrx(bp.gpu_diff, bp.gpu_diff)
                    # history2.data = history2.data * mom2 + param.diff
                    axpby(DTYPE(self.mom2), bh2.gpu_data,
                          DTYPE(1.), bp.gpu_diff, bh2.gpu_data)
                    # param.diff = history2.data / accum_mult2
                    axpb(DTYPE(1./self.accum_mult2), bh2.gpu_data,
                         DTYPE(0.), bp.gpu_diff)
                    # param.diff = sqrt(param.diff)
                    sqrtx(bp.gpu_diff, bp.gpu_diff)
                    # param.diff = history.data / (param.diff + eps)
                    xdivyeps(bh.gpu_data, bp.gpu_diff,
                             DTYPE(self.eps), bp.gpu_diff)
                    # param.diff = param.diff / accum_mult
                    axpb(DTYPE(1./self.accum_mult), bp.gpu_diff,
                         DTYPE(0.), bp.gpu_diff)
                    # param.data = param.data - param.diff * lr
                    axpby(DTYPE(1.), bp.gpu_data,
                          DTYPE(-self.lr), bp.gpu_diff, bp.gpu_data)
        
    def _update_adam(self, param, history, history2):
        self._update_adam_gpu(param, history, history2)
	...
	if self.solver_type == 'SGD':
            ...
            self._update_rmsprop(param, history2)
        elif self.solver_type == 'Adam':
            self._update_adam(param, history, history2)
        else:
    ...
    def step(self, verbose=False):
        ...
        for net in self.nets:
            net.backward()
        # update
        for (param, hist, hist2) in zip(self.params, self.historys, self.history2s):
